...
insert into ['2019']
	select [code_sub],[name_sub], [year], [code_ou], 
		...
		(select Count(*) as [Количество участников]
				...
				,(select Count([Зачет]) from [dbo].[ege2019] where [Код предмета] = 2 and [Зачет] = 2 and #Sepjson.[code_ou] = [dbo].[ege2019].[Код ОУ]) as [Не преодолели минимального порогового значения]
				,(select Count([Балл]) from [dbo].[ege2019] where [Код предмета] = 2 and [Балл] = 100 and #Sepjson.[code_ou] = [dbo].[ege2019].[Код ОУ]) as [Количество 100-балльников]
