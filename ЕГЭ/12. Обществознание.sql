...
set [1] = substring(part1,1,1),[2] = substring(part1,2,1),[3] = substring(part1,3,1),
	...
	[19] = substring(part1,19,1),[20] = substring(part1,20,1),
...
,[c17],[c18],[c19],[c20],[sc1],[sc2],[sc3],[sc4],[sc5],[sc6],[sc7],[sc8],[sc9]) 
as (select [cluster]
	,round((sum([1])/Count([1]))/1*100, 1,1) as [1]
	  ...
	  , round((sum([s9])/Count([s9]))/6*100, 1,1) as [s9]
	from #TempEge
	group by [cluster]
	)
...
,[r17],[r18],[r19],[r20],[sr1],[sr2],[sr3],[sr4],[sr5],[sr6],[sr7],[sr8],[sr9]) 
as (select
		round((sum([1])/Count([1]))/1*100, 1,1) as [1]
	  , round((sum([2])/Count([2]))/1*100, 1,1) as [2]
	  , round((sum([3])/Count([3]))/1*100, 1,1) as [3]
...
select [code_sub],[name_sub], [year], [code_ou], 
	 ...
	 , round((sum([1])/Count([1]))/1*100, 1,1) as [s1]
	  ...
	  , round((sum([s9])/Count([s9]))/6*100, 1,1) as [ss9]
...
select [code_sub],[name_sub], [year], [code_ou], 
		...
		,(select 'Осуществлять поиск социальной информации; извлекать из неадаптированных оригинальных текстов (правовых, научнопопулярных, публицистических и др.) знания по заданным темам; систематизировать, анализировать и обобщать неупорядоченную социальную информацию ' as specific, CONVERT(NUMERIC(5,1), ROUND([ss1], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc1], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr1], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s1]
		,(select 'Осуществлять поиск социальной информации; извлекать из неадаптированных оригинальных текстов (правовых, научнопопулярных, публицистических и др.) знания по заданным темам; систематизировать, анализировать и обобщать неупорядоченную социальную информацию.Объяснять внутренние и внешние связи (причинно-следственные и функциональные) изученных социальных объектов 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss2], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc2], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr2], 1, 1)) 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s2]
		,(select 'Объяснять внутренние и внешние связи (причинно-следственные и функциональные) изученных социальных объектов. Раскрывать на примерах изученные теоретические положения и понятия социальноэкономических и гуманитарных наук 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss3], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc3], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr3], 1, 1)) 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s3]
		,(select 'Объяснять внутренние и внешние связи (причинно-следственные и функциональные) изученных социальных объектов. Оценивать действия субъектов социальной жизни, включая личность, группы, организации, с точки зрения социальных норм, экономической рациональности Формулировать на основе приобретенных обществоведческих знаний собственные суждения и аргументы по определенным проблемам 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss4], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc4], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr4], 1, 1)) 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s4]
		,(select 'Характеризовать с научных позиций основные социальные объекты ( факты, явления, процессы, институты), их место и значение в жизни общества как целостной системы (задание на раскрытие смысла понятия, использование понятия в заданном контексте) 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss5], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc5], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr5], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s5]
		,(select 'Раскрывать на примерах изученные теоретические положения и понятия социальноэкономических и гуманитарных наук (задание, предполагающее раскрытие теоретических положений на примерах)'as specific,CONVERT(NUMERIC(5,1), ROUND([ss6], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc6], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr6], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s6]
		,(select 'Применять социальноэкономические и гуманитарные знания в процессе решения познавательных задач по актуальным социальным проблемам ( заданиезадача) 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss7], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc7], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr7], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s7]
		,(select 'Подготавливать аннотацию, рецензию, реферат, творческую работу (задание на составление плана доклада по определенной теме) 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss8], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc8], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr8], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s8]
		,(select 'Характеризовать с научных позиций основные социальные объекты ( факты, явления, процессы, институты), их место и значение в жизни общества как целостной системы. Анализировать актуальную информацию о социальных объектах,выявляя их общие черты и различия; устанавливать соответствия между существенными чертами и признаками изученных социальных явлений и обществоведческими терминами и понятиями. Объяснять внутренние и внешние связи (причинно-следственные и функциональные) изученных социальных объектов. Раскрывать на примерах изученные теоретические положения и понятия социальноэкономических и гуманитарных наук. Оценивать действия субъектов социальной жизни, включая личность, группы, организации, с точки зрения социальных норм, экономической рациональности. Формулировать на основе приобретенных обществоведческих знаний собственные суждения и аргументы по определенным проблемам 'as specific,CONVERT(NUMERIC(5,1), ROUND([ss9], 1, 1)) as 'school', CONVERT(NUMERIC(5,1), ROUND([sc9], 1, 1)) as 'cluster', CONVERT(NUMERIC(5,1), ROUND([sr9], 1, 1)) as 'region' from #Full where #TempSchool.[code_ou] = #Full.[code_ou] for json path, WITHOUT_ARRAY_WRAPPER  ) as [s9]
...
insert into ['2020']
	...
	select [code_sub],[name_sub], [year], [code_ou], 
		...
		(select Count(*) as [Количество участников]
				...
				,(select Count([Зачет]) from [dbo].[ege2020] where [Код предмета] = 12 and [Зачет] = 2 and #Sepjson.[code_ou] = [dbo].[ege2020].[Код ОУ]) as [Не преодолели минимального порогового значения]
				,(select Count([Балл]) from [dbo].[ege2020] where [Код предмета] = 12 and [Балл] = 100 and #Sepjson.[code_ou] = [dbo].[ege2020].[Код ОУ]) as [Количество 100-балльников]
		
		from [dbo].[ege2020] 
		where [Код предмета] = 12 and
